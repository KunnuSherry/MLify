...
const DecayCard = ({ width = 300, height = 400, image = 'https://picsum.photos/300/400?grayscale', children }) => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Ease back to neutral when not hovering
      imgValues.imgTransforms.x = isHoveringRef.current ? targetX : lerp(imgValues.imgTransforms.x, 0, 0.12);
      imgValues.imgTransforms.y = isHoveringRef.current ? targetY : lerp(imgValues.imgTransforms.y, 0, 0.12);
      imgValues.imgTransforms.rz = isHoveringRef.current ? targetRz : lerp(imgValues.imgTransforms.rz, 0, 0.12);
      ...
      const targetScale = isHoveringRef.current ? map(cursorTravelledDistance, 0, 200, 0, 400) : 0;
      imgValues.displacementScale = lerp(imgValues.displacementScale, targetScale, 0.06);
